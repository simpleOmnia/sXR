    ...
    void Update() {
        switch (sxr.GetPhase()) {
            case 0: // Start Screen Phase
                sxr.DisplayImage("loading", sxr.UI_Position.FullScreen1);
                break; 
            ...
            case 3: // Testing Round
                sxr.DisplayText(GazeHandler.Instance.GetScreenFixationPoint().x+", "+GazeHandler.Instance.GetScreenFixationPoint());
                switch (sxr.GetStepInTrial())
                {
                    ...
                    case 2: // Runs until CheckTimer()==10 -- Looking for sphere in box
                        ...
                        if (sxr.CheckCollision("Sphere", "TargetBox")) {
                            numHits++;
                            sxr.PlaySound(sxr.ProvidedSounds.Ding);
